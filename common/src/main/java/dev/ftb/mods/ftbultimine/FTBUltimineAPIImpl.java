...
public enum FTBUltimineAPIImpl implements FTBUltimineAPI.API {
    ...
    @Override
    public BlockSelectionHandler.Result customSelectionCheck(Level level, BlockPos origPos, BlockPos pos, BlockState origState, BlockState state) {
        for (var h : BlockSelectionRegistry.INSTANCE.getHandlers()) {
            var res = h.customSelectionCheck(level, origPos, pos, origState, state);
